...
func addQuestionsForTopics() error {
	// История России
	query := `
    INSERT INTO questions (topic_id, question_text, answers, correct_option_index, hard_level) VALUES
	...
	DO UPDATE SET
		...
		hard_level = EXCLUDED.hard_level;`

	_, err := DB.Exec(query)
	if err != nil {
		return fmt.Errorf("failed to insert questions for : %v", err)
	}
